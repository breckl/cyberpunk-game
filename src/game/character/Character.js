...
class Character {
  constructor(name, characterClass) {
    ...
    this.abilities = classes[characterClass].specialAbilities.map(ability => ({
      ...ability,
      currentCooldown: 0
    }));
  ...
  useAbility(abilityName, target) {
    ...
    if (!this.useEnergy(ability.energyCost)) return { success: false, message: 'Not enough energy' };
  ...
  installCybernetic(cybernetic) {
    ...
    const existingInSlot = this.cybernetics.find(c => c.slot === cybernetic.slot);
    if (existingInSlot) {
